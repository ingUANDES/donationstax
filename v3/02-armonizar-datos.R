...
d <- d %>%
  mutate(donante.x = ifelse(donante.x %in% c("NULO", ".. ... ...", "... ... ..."), NA, donante.x))

d <- d %>%
  mutate(donante.x = ifelse(donante.x %in% c("NULO", ".. ... ...", "... ... ..."), NA, donante.x))

d %>%
  filter(grepl("\\.\\.", donante.x)) %>%
  select(donante.x)
...
drut <- d %>%
  filter(!is.na(as.numeric(str_sub(donante.x, 1, 1))))

drut %>%
  mutate(
    donante.x_rut = case_when(
      as.numeric(str_sub(donante.x, 1, 1)) == rutnum_dv(donante.x) ~ rut_rut10(donante.x),
      TRUE ~ NA
    )
  )
